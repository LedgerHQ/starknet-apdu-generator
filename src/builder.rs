use crate::apdu::{Apdu, Ins, FieldElement, CallArray};
        ...
        Some(spath) => {
            for s in spath.split('/') {
                let val: u32 = match s.ends_with("'") {
                    true => {
                        0x80000000 + s.strip_suffix("'").unwrap().parse::<u32>().unwrap()
                    }
                    false => {
                        s.parse::<u32>().unwrap()
                    }
...
pub fn build_metadata_apdu(ins: Ins, aa: &str, max_fee: &str, chain_id: &str, nonce: &str, version: &str) -> Apdu {
...
pub fn build_calldata_apdu(ins: Ins, calls: &[CallArray]) -> Vec<Apdu> {
    ...
    apdu.ins = ins;
    apdu.p1 = 0x02;
    let call_array_len: FieldElement = FieldElement(U256::from_big_endian(calls.len().to_be_bytes().as_slice()));
    ...
    let calldata_len: FieldElement = FieldElement(U256::from_big_endian(total_calldata_len.to_be_bytes().as_slice()));
    ...
    for (pos, c) in calls.iter().enumerate() {
        ...
        let data_offset: FieldElement = FieldElement(U256::from_big_endian(offset.to_be_bytes().as_slice()));
        data = data_offset.try_into().unwrap();
        apdu.append(data.as_slice()).unwrap();
        
        let data_len: FieldElement = FieldElement(U256::from_big_endian(c.calldata.len().to_be_bytes().as_slice()));
